...
int test_basic_operations()
{
    ...
    // 插入测试
    TEST_RESULT("在位序 1 插入 10", SeqList_InsertItem(&seq, 1, 10), SeqList_Success);
    TEST_RESULT("在位序 2 插入 20", SeqList_InsertItem(&seq, 2, 20), SeqList_Success);
    TEST_RESULT("在位序 1 插入 5 (头部)", SeqList_InsertItem(&seq, 1, 5), SeqList_Success);
    ...
    TEST("查找不存在的元素返回 -3", SeqList_OrdOfItem(&seq, 999) == SeqList_Error_ITEM_NOT_FOUND);
...
int test_boundary_conditions()
{
    ...
    // 越界插入测试
    TEST_RESULT("在位序 0 插入 (越界)", SeqList_InsertItem(&seq, 0, 1), SeqList_Error_ORD_OUT_OF_RANGE);
    TEST_RESULT("在位序 2 插入空表 (越界)", SeqList_InsertItem(&seq, 2, 1), SeqList_Error_ORD_OUT_OF_RANGE);

    // 插入一个元素后测试越界
    SeqList_InsertItem(&seq, 1, 100);
    TEST_RESULT("在位序 3 插入 (越界)", SeqList_InsertItem(&seq, 3, 1), SeqList_Error_ORD_OUT_OF_RANGE);

    // 越界获取/修改/删除
    TEST_RESULT("获取位序 0 (越界)", SeqList_GetItem(&seq, 0, &item), SeqList_Error_ORD_OUT_OF_RANGE);
    TEST_RESULT("获取位序 2 (越界)", SeqList_GetItem(&seq, 2, &item), SeqList_Error_ORD_OUT_OF_RANGE);
    TEST_RESULT("修改位序 0 (越界)", SeqList_PutItem(&seq, 0, 1, NULL), SeqList_Error_ORD_OUT_OF_RANGE);
    TEST_RESULT("删除位序 0 (越界)", SeqList_DeleteItem(&seq, 0, &item), SeqList_Error_ORD_OUT_OF_RANGE);
    ...
    TEST("NULL 查找元素", SeqList_OrdOfItem(NULL, 1) == SeqList_Error_NULL_SEQ);
    ...
    SeqList_Destroy(&seq);
    SeqList_Initialize(&seq);
    TEST("空表查找返回未找到", SeqList_OrdOfItem(&seq, 1) == SeqList_Error_ITEM_NOT_FOUND);

    // 可选参数测试（item 可为 NULL）
    TEST_RESULT("获取元素但 item 为 NULL", SeqList_GetItem(&seq, 1, NULL), SeqList_Error_ORD_OUT_OF_RANGE); // 先插入一个
    SeqList_InsertItem(&seq, 1, 42);
    TEST_RESULT("获取元素但 item 为 NULL (有效位序)", SeqList_GetItem(&seq, 1, NULL), SeqList_Success);
...
int test_expansion()
{
    ...
    {
        SeqList_Result r = SeqList_InsertItem(&seq, ord, ord * 10);
